...
<template>
  <div class="flex gap-x-4 justify-between items-center relative">
    ...
    <div ref="scrubber" class="w-full relative z-9990 h-4 rounded-sm cursor-pointer select-none bg-zinc-600" @mousedown="scrubbing = true">
      <div class="relative w-full h-4 overflow-hidden">
        <div class="absolute w-full h-4 opacity-30 left-0 top-0 rounded-sm bg-zinc-900" :style="{ transform: `translateX(${(secondary / max) * 100 - 100}%)` }" />
        <div class="relative w-full h-4 rounded-sm bg-zinc-300" :style="{ transform: `translateX(${(value / max) * 100 - 100}%)` }" />
      </div>
