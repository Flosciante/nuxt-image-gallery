  ...
  <div class="flex justify-between items-center relative">
    ...
    <div ref="scrubber" class="w-full relative z-9990 h-[27px] rounded-l-md cursor-pointer select-none bg-transparent border-l border-t border-b border-1 border-gray-700" @mousedown="scrubbing = true">
      <div class="relative w-full overflow-hidden flex">
        <div class="absolute w-full opacity-30 left-0 top-0 rounded-sm bg-gray-900" :style="{ transform: `translateX(${(secondary / max) * 100 - 100}%)` }" />
        <div class="relative w-full rounded-sm bg-gray-300 h-[23px] mt-[1px]" :style="{ transform: `translateX(${(value / max) * 100 - 100}%)` }" />
      </div>
